...
class MusicBrainzIndex(Index):
	def __init__(self,dbname,user,pw):
		super(MusicBrainzIndex,self).__init__(dbname,user,pw)
	...
	def _create_file(res,i,fs):
		...
		while True:
			i = MusicBrainzIndex._create_tag(res,i,ts)
	...
	def _create_files_from_flat_list(res):
		...
		i = 0
		while i<len(res):
			i = MusicBrainzIndex._create_file(res,i,fs)
	...
	def get_files_with_tags_and_puid(self):
		...
		fs = MusicBrainzIndex._create_files_from_flat_list(res)
		return fs
