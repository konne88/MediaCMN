...
class MusicBrainzOptions(CommonCmnOptions):
	def __init__(self):
		super(MusicBrainzOptions,self).__init__()
		self.level = 0.0
		...
		self._appdesc=("Find tags for a music file using music brainz.")
	...
	def _set_option_value(self,opt,val):
		...
		if opt == 'level':
			...
			r = super(MusicBrainzOptions,self)._set_option_value(opt,val)
			if r != None:
				q = r
		return q
