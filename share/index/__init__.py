...
class IndexReference(object):
    """Represents all data needed to succesfully connect to a specific db."""
    def __init__(self,path):
        self.path = path

class _SqliteCursorWrapper(sqlite3.Cursor):
    """This is a wrapper class for sqlite cursors, cause this class
    needs ? instead of %s. See the Cursor definition for function
    descriptions.
    """
...
class Index(object):
    ...
    def _add_hash(self,value,table):
        """Generic function to add a hash to the index."""
    ...
    def get_file_taggroups(self,fileid):
        """Returns the `TagGroup` belonging to this `fileid`."""
        self._cursor.execute(u'''
            SELECT 
                g.id AS groupid,
                t.source AS source,
